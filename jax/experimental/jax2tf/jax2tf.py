...
class _ThreadLocalState(threading.local):
  def __init__(self):
    ...
    # Experimental flag to use the JAX native lowering to MHLO for each
    # primitive.
...
def convert(fun: Callable,
            *,
            ...
            with_gradient=True,
            enable_xla=True,
            experimental_native_lowering=False) -> Callable:
  ...
  def converted_fun(*args: TfVal, **kwargs: TfVal) -> TfVal:
    ...
    try:
      ...
      _thread_local_state.experimental_native_lowering = True  # experimental_native_lowering
...
def _interpret_fun(
  ...
  if _thread_local_state.experimental_native_lowering:
    ...
    abstract_axes: Sequence[Dict[int, str]] = []
    has_shape_vars = False
    for aval in in_avals:
      ...
      for i, d in enumerate(aval.shape):
        if not core.is_constant_dim(d):
          ...
          has_shape_vars = True
          aval_abstract_axes[i] = d_var
      abstract_axes.append(aval_abstract_axes)
    if has_shape_vars:
      if not config.jax_dynamic_shapes:
        raise ValueError(
            f"Found shape polymorphism but --jax_dynamic_shapes is not on")
      ...
      # This is a hack, find a way to refactor infer_input_type so that we
      # can reuse it here more cleanly.
      top_trace = core.find_top_trace(())
      fake_jax_vals = [
          TensorFlowTracer(top_trace, val, aval)
          for val, aval in zip(in_vals, in_avals)
      ]
      in_type = pe.infer_lambda_input_type(abstract_axes, fake_jax_vals)
      ...
      dim_var_specs_dict: Dict[int, str] = {}
      for arg_idx, (aval, is_explicit) in enumerate(in_type):
        if not is_explicit:
          nr_dim_vars += 1
        else:
          for i, d in enumerate(aval.shape):
            if isinstance(d, core.DBIdx) and d.val not in dim_var_specs_dict:
              dim_var_specs_dict[d.val] = f"{arg_idx - nr_dim_vars}.{i}"
      dim_var_specs = [dim_var_specs_dict[i] for i in range(nr_dim_vars)]
    else:
      arg_specs = [(aval, None) for aval in in_avals]
      dim_var_specs = []
    ...
    logging.info(f"jax2tf XlaCallModule {mhlo_module_text}")
