...
def pjit(fun: Callable,
  ...
  def infer_params(*args, _global_avals=False, **kwargs):
    ...
    if config.jax_array:
      ...
      # TODO(yashkatariya): Make out_axis_resources take `Sharding` objects too.
      out_shardings = tree_map(lambda x: _create_mesh_pspec_sharding(pjit_mesh, x), out_axis_resources)
...
def _pjit_call_impl(*args, jaxpr,
  ...
  # TODO(yashkatariya): Don't check `o.spec` on output shardings.
  if config.jax_array and all(_is_unspecified(o.spec) for o in out_shardings):
...
def _pjit_lower(
  ...
  out_axes = [get_array_mapping(o._canonicalized_spec) for o in out_shardings]
...
def _pjit_abstract_eval(*args, jaxpr, out_shardings, resource_env,
  ...
  return global_to_local(out_positional_semantics, resource_env.physical_mesh,
                         jaxpr.out_avals, out_shardings), jaxpr.effects
...
def _pjit_transpose(reduce_axes, cts_in, *primals_in,
  ...
  if not config.jax_array:
    ...
    global_cts_in_avals = local_to_global(
        transpose_in_positional_semantics, mesh, global_cts_in_avals,
        transpose_in_shardings)
...
def with_sharding_constraint(x, axis_resources):
  ...
  outs = [sharding_constraint_p.bind(xf, in_sharding=i, resource_env=resource_env)
          for xf, i in safe_zip(x_flat, in_sharding_flat)]
  return tree_unflatten(tree, outs)

def _sharding_constraint_impl(x, in_sharding, resource_env):
...
ad.deflinear2(sharding_constraint_p,
              lambda ct, _, in_sharding, resource_env: (
                  sharding_constraint_p.bind(
                      ct, in_sharding=in_sharding, resource_env=resource_env),))
...
def _sharding_constraint_batcher(insert_axis, axis_size, axis_name, main_type,
                                 vals_in, dims_in, in_sharding, resource_env):
  x, = vals_in
  d, = dims_in
...
def _resource_typing_sharding_constraint(avals, params, source_info,
  ...
  _check_resources_against_named_axes(
    ...
    params['in_sharding']._canonicalized_spec, named_axis_resources)
...
def get_unconstrained_dims(in_sharding: sharding.MeshPspecSharding):
  return {i for i, axes in enumerate(in_sharding._canonicalized_spec)
          if axes is None}


def global_to_local(positional_semantics, mesh, avals, shardings):
  if config.jax_array:
    return avals
  ...
  return [
      ...
      s._canonicalized_spec.partitions == () else mesh._global_to_local(
          get_array_mapping(s._canonicalized_spec), aval)
      for aval, s, ps in safe_zip(avals, shardings, positional_semantics)
  ]

def local_to_global(positional_semantics, mesh, avals, shardings):
  if config.jax_array:
    return avals
  return [
      aval if ps == maps._PositionalSemantics.GLOBAL or
      s._canonicalized_spec.partitions == () else mesh._local_to_global(
          get_array_mapping(s._canonicalized_spec), aval)
      for aval, s, ps in safe_zip(avals, shardings, positional_semantics)
  ]
