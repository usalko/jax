...
# `OpShardingSharding`.
def _pjit_batcher_for_sharding(s, dim, val, mesh, ndim):
...
def _replicated_op_sharding_sharding(da):
  proto = xc.OpSharding()
  proto.type = xc.OpSharding.Type.REPLICATED
  return OpShardingSharding(da, proto)
...
def _pjit_partial_eval(trace, *in_tracers,
                       ...
                       out_positional_semantics):
  da = None
  for i in in_shardings + out_shardings:
  ...
  known_params = dict(
      ...
      out_shardings=(
          ...
          (_replicated_op_sharding_sharding(da),) * num_residuals),
      resource_env=resource_env,
...
ad.deflinear2(sharding_constraint_p,
              lambda ct, _, sharding, resource_env, unconstrained_dims: (
...
def _sharding_constraint_mhlo_lowering(ctx, x_node, *, sharding,
  ...
  if hasattr(axis_ctx, 'manual_axes'):
    mesh = resource_env.physical_mesh
...
def global_to_local(positional_semantics, avals, shardings, mesh):
  ...
  for aval, s, ps in safe_zip(avals, shardings, positional_semantics):
    if (ps == maps._PositionalSemantics.GLOBAL or
        s._op_sharding.type == xc.OpSharding.Type.REPLICATED):
      out.append(aval)
    else:
...
def _calc_is_global_sequence(in_positional_semantics, in_shardings):
  ...
  return tuple((ips == maps._PositionalSemantics.GLOBAL or
                i._op_sharding.type == xc.OpSharding.Type.REPLICATED)
