  ...
  def infer_params(*args, _global_avals=False, **kwargs):
    ...
    if config.jax_array:
      in_shardings, out_shardings = _get_and_check_in_and_out_shardings(
          dyn_args, in_axis_resources, out_axis_resources, pjit_mesh, in_tree)
    else:
...
def _get_and_check_in_and_out_shardings(args, pjit_in_shardings, out_shardings,
                                        pjit_mesh, in_tree):
  # tree_map over `args` to preserve the pytree structure of args.
  arg_in_shardings = tree_map(
      lambda x: x.sharding if hasattr(x, 'sharding') else _UNSPECIFIED, args)
  ...
  if _is_unspecified(pjit_in_shardings):
    ...
    for a in arg_in_shardings_flat:
      if _is_unspecified(a):
        raise ValueError('Please specify sharding either on the args or on '
                         f'pjit. Found arg {a} with no sharding specification.')
    in_shardings = arg_in_shardings
  else:
    # This function is cached.
    in_shardings = _get_and_check_pjit_arg_shardings(
        hashable_pytree(pjit_in_shardings), arg_in_shardings_flat, in_tree)

  in_shardings_flat = tuple(tree_flatten(in_shardings)[0])
...
def _get_and_check_pjit_arg_shardings(pjit_in_shardings, arg_in_shardings_flat,
  ...
  out = []
  for pjit_sharding, arg_sharding in safe_zip(pjit_in_shardings_flat, arg_in_shardings_flat):
