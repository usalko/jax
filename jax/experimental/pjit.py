...
def pjit(fun: Callable,
  ...
  def infer_params(*args, _global_avals=False, **kwargs):
    ...
    # Don't allow any type other than GDA and ShapedArray for auto sharding.
    if in_all_auto:
      assert all(isinstance(a, GDA) or isinstance(a, core.ShapedArray) for a in args_flat), args_flat
...
def _prepare_axis_resources(axis_resources,
  ...
  all_auto = pxla._check_if_all_auto(entries, arg_name)
  if not all_auto:
    entries = [
