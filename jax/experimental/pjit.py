...
def pjit(fun: Callable,
  ...
  if config.jax_array and _is_unspecified(in_axis_resources):
    ...
    if _is_unspecified(out_axis_resources):
      raise ValueError('Please specify `out_axis_resources` when using `Array` '
                       'for now until we add some support to support unspecifed '
                       'out_axis_resources.')
  ...
  def infer_params(*args, _global_avals=False, **kwargs):
    ...
    global_in_avals, canonicalized_in_axis_resources_flat = _process_in_axis_resources(
        ...
        tuple(isinstance(a, GDA) or isinstance(a, Array) for a in args_flat))
...
def _maybe_replace_from_gda_with_pspec(
  ...
  if isinstance(arg, Array):
    return CanonicalizedParsedPartitionSpec(ParsedPartitionSpec.from_user_input(
        arg.sharding.spec, arg_name="GDA mesh_axes"))
