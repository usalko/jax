...
def bcoo_dot_general(lhs, rhs, *, dimension_numbers):
  ...
  return _bcoo_dot_general(*lhs._bufs, rhs, dimension_numbers=(cdims, bdims),
...
def bcoo_rdot_general(lhs, rhs, *, dimension_numbers: DotDimensionNumbers):
  result = _bcoo_dot_general(rhs.data, rhs.indices, lhs, lhs_spinfo=rhs._info,
                             dimension_numbers=[d[::-1] for d in dimension_numbers])
  n_contract, n_batch = (len(d[0]) for d in dimension_numbers)
  n_swap = len(rhs._info.shape) - n_contract
...
def bcoo_broadcast_in_dim(mat, *, shape, broadcast_dimensions):
  return _bcoo_broadcast_in_dim(mat.data, mat.indices, spinfo=mat._info,
...
def bcoo_reduce_sum(mat, *, axes):
  return _bcoo_reduce_sum(mat.data, mat.indices, spinfo=mat._info, axes=axes)
...
def bcoo_multiply_sparse(lhs, rhs):
  return _bcoo_multiply_sparse(lhs.data, lhs.indices, rhs.data, rhs.indices,
                               lhs_spinfo=lhs._info, rhs_spinfo=rhs._info)
...
class BCOO(JAXSparse):
  ...
  def transpose(self, axes=None):
    ...
    data_T, indices_T = _bcoo_transpose(self.data, self.indices, permutation=axes, spinfo=self._info)
