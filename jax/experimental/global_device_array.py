...
def _get_shard_indices_replica_ids_uncached(
  ...
  expected_unique_rid = prod(
      [g / s for g, s in safe_zip(global_shape, shard_shape)])
  if expected_unique_rid != unique_rid:
    raise RuntimeError(
        f'Number of expected unique replica ids are: {expected_unique_rid} but '
        f'got {unique_rid}. Please file a bug at '
