...
def _outside_call_lowering(
    ctx: mlir.LoweringRuleContext, *args,
    has_token: bool, identity: bool,
    flat_results_aval=(),
    **params):
  ...
  callback_operands = [current_token, replica_id, *args_to_outfeed]
  callback_operand_avals = [core.abstract_token, core.ShapedArray((),
    np.uint32), *ctx.avals_in[:-2]]
  if identity:
    callback_flat_results_aval = [core.abstract_token]
  else:
    callback_flat_results_aval = [core.abstract_token, *flat_results_aval]
  ...
  result = mhlo.CustomCallOp([result_type], callback_operands,
      ...
      result_layouts=None)
  results = [
    mhlo.GetTupleElementOp(result, ir.IntegerAttr.get(i32_type, i)).result
...
if jaxlib.version >= (0, 3, 8):
  _outside_call_outfeed_rule = mlir.xla_fallback_lowering(outside_call_p)
  mlir.register_lowering(outside_call_p, _outside_call_outfeed_rule, platform="gpu")
  mlir.register_lowering(outside_call_p, _outside_call_outfeed_rule, platform="tpu")
