...
def _full_to_shard_lowering(ctx, x, *, axes: ArrayMapping, mesh: Mesh, manual_axes: Sequence[ArrayMapping]):
  ...
  manual_proto = _manual_proto(aval_in, axes if manual_axes is None else manual_axes, mesh)
...
def _shard_to_full_lowering(ctx, x, *, axes: ArrayMapping, mesh: Mesh, manual_axes: Sequence[ArrayMapping]):
  aval_in, = ctx.avals_in
  aval_out, = ctx.avals_out
  manual_proto = _manual_proto(aval_in, axes if manual_axes is None else manual_axes, mesh)
  result_type, = mlir.aval_to_ir_types(aval_out)
...
@lu.transformation
def vtile_manual(manual_axes: Sequence[MeshAxisName],
                 mesh: Mesh,
                 in_axes: Sequence[ArrayMapping],
                 out_axes: Sequence[ArrayMapping],
                 *args):
  ...
  if spmd_lowering:
    ...
    if tiling_method is not None:
      if isinstance(tiling_method, TileVectorize):
        ...
        tiling_transform = lambda f, *args: vtile_manual(f, tiling_method.manual_axes, *args)  # typing: ignore
      else:
