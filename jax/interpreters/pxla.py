...
def lower_sharding_computation(
  ...
  if traced_jaxpr_info is None:
    with dispatch.log_elapsed_time(f"Finished tracing + transforming {name_stack} "
      ...
      jaxpr, out_jaxpr_avals, consts = pe.trace_to_jaxpr_final(
          fun, global_in_avals, debug_info=pe.debug_info_final(fun, "sharding computation"))
  else:
  ...
  in_op_shardings = [
      None if isinstance(aval, core.AbstractToken) else i._to_xla_op_sharding(aval.ndim)
  ...
  out_op_shardings = [
      None if _is_unspecified(o) or isinstance(aval, core.AbstractToken) else o._to_xla_op_sharding(aval.ndim)
  ...
  if len(first_sharding._device_assignment) > 1:
    if any(eff in core.ordered_effects for eff in closed_jaxpr.effects):
