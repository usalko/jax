...
class RuntimeTokenSet(threading.local):
  ...
  output_tokens: Dict[Tuple[core.Effect, Device], RuntimeToken]
  ...
  def set_output_token(self, eff: core.Effect, device: Device,
                       token: RuntimeToken):
    self.output_tokens[eff, device] = token
...
def _add_tokens(unordered_effects: List[core.Effect], ordered_effects: List[core.Effect], device, input_bufs):
  ...
  def _remove_tokens(output_bufs):
    output_token_bufs, token_bufs, output_bufs = util.split_list(
        output_bufs, [len(unordered_effects),
                      len(ordered_effects)])
    for eff, token_buf in zip(unordered_effects, output_token_bufs):
      runtime_tokens.set_output_token(eff, device, token_buf)
...
def _execute_compiled(name: str, compiled: XlaExecutable,
  ...
  if unordered_effects or ordered_effects:
    input_bufs_flat, token_handler = _add_tokens(
        unordered_effects, ordered_effects, device, input_bufs_flat)
...
class XlaCompiledComputation(stages.Executable):
  ...
  def from_xla_computation(
    ...
    if ordered_effects or unordered_effects:
      if buffer_counts is None:
        buffer_counts = [1]
      buffer_counts = ([1] * len(unordered_effects + ordered_effects)) + buffer_counts
    ...
    unsafe_call = partial(execute, name, compiled, input_handler, buffer_counts,  # type: ignore  # noqa: F811
                          result_handlers, unordered_effects, ordered_effects,
                          kept_var_idx)
  ...
  def from_trivial_jaxpr(jaxpr, consts, device, in_avals, out_avals,
    ...
    unsafe_call = partial(_execute_trivial, jaxpr, device, consts,
                          out_avals, result_handlers, unordered_effects,
