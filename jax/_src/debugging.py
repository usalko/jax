...
def _debug_callback_partial_eval_custom(saveable, unks_in, inst_in, eqn):
  # The default behavior for effectful primitives is to always be saved if
  # possible. For debug callback, we actually want it to be rematerialized to
  # provide more information to the user. This rule bypasses partial_eval's
  # regular behavior to do that. Specifically, we will rematerialize the print
  # if:
  # 1) the policy does not allow debug_callbacks to be saved
  # 2) the policy does allow debug_callbacks to be saved BUT all of the input
  #    values are instantiated.
  # The purpose is to call back with as much information as possible while
  # avoiding adding additional rematerializations.
  if any(unks_in):
