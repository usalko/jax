...
class _Stack:
  ...
  def __repr__(self):
    return f"_Stack({self._size}, {self._data})"

  @staticmethod
  def create(capacity: int, prototype: Any) -> _Stack:
  ...
  def pop(self) -> Tuple[Any, _Stack]:
    ...
    elem = jax.tree_util.tree_map(
      ...
      self._data)
    return elem, _Stack(self._size - 1, self._data)
  ...
  def unflatten(treedef, leaves):
    return _Stack(leaves[0], jax.tree_util.tree_unflatten(treedef, leaves[1:]))

jax.tree_util.register_pytree_node(_Stack, _Stack.flatten, _Stack.unflatten)
...
def _slice(operand, start_indices, dynamic_slice_sizes, static_slice_sizes,
  ...
  out = lax.dynamic_slice(padded, start_indices, static_slice_sizes)
  return _mask(out, dynamic_slice_sizes, fill_value)

def _update_slice(operand, update, start_indices, update_dims):
  """update_dims are the true dimensions of the padded update `update`"""
...
def _similarity_transform(matrix_in, matrix_out):
  """ Returns matrix_out.conj().T @ matrix_in @ matrix_out, done in
  order from left to right.
  """
  out = jnp.dot(matrix_out.conj().T, matrix_in)
  return jnp.dot(out, matrix_out)
...
def split_spectrum(H, n, split_point, V0=None):
  ...
  Vm, Vp = _projector_subspace(P, H, n, rank)
  Hm = _similarity_transform(H, Vm)
  Hp = _similarity_transform(H, Vp)
  if V0 is not None:
    Vm = jnp.dot(V0, Vm)
    Vp = jnp.dot(V0, Vp)
  return Hm, Vm, Hp, Vp, rank
...
#   Hm, Vm, Hp, Vp = split_spectrum(H, split_point, V0=V)
#   Hm, Vm = _eigh_work(Hm, V=Vm, termination_size=termination_size)
#   Hp, Vp = _eigh_work(Hp, V=Vp, termination_size=termination_size)
#
...
def _eigh_work(H, n, termination_size=256):
  ...
  N, _ = H.shape
  agenda = _Stack.create(
    N + 1, _Subproblem(jnp.array(0, jnp.int32), jnp.array(0, jnp.int32)))
  agenda = agenda.push(_Subproblem(offset=0, size=n))
  ...
  eigenvectors = _mask(eigenvectors, (n, n))  # needed?
  ...
  # column). Here is an ASCII art picture of three blocks A, B, C:
  #
  # A A A ....
  # A A A ....
  # A A A ....
  ...
  # C C C C ...
  #
  # Each step of the algorithm subdivides a block into two subblocks whoses
  ...
  def base_case(B, offset, b, agenda, blocks, eigenvectors):
    ...
    # of the eigenvalues, so we can obtain the eigendecomposition of the real
    # matrix in the top-left corner of the eigendecomposition of the padded
    # matrix.
    ...
    evecs, evals = lax.linalg.eigh(H, sort_eigenvalues=False)
    evecs = _mask(evecs, (b, b))
    evals = _mask(evals, (b,))
    evecs = jnp.dot(V, evecs)

    blocks = _update_slice(blocks, evals[:, None], (offset, 0), (b, b))
    eigenvectors = _update_slice(eigenvectors, evecs, (0, offset), (n, b))
  ...
  def recursive_case(B, offset, b, agenda, blocks, eigenvectors):
    ...
    Hm, Vm, Hp, Vp, rank = split_spectrum(H, b, split_point, V0=V)

    blocks = _update_slice(blocks, Hm, (offset, 0), (rank, rank))
    blocks = _update_slice(blocks, Hp, (offset + rank, 0), (b - rank, b - rank))
    eigenvectors = _update_slice(eigenvectors, Vm, (0, offset), (n, rank))
    eigenvectors = _update_slice(eigenvectors, Vp, (0, offset + rank),
                                 (n, b - rank))
...
def eigh(H, *, precision="float32", termination_size=256, n=None):
  ...
  with jax.default_matmul_precision(precision):
    evals, evecs = _eigh_work(H, n, termination_size=termination_size)
  evals = _mask(evals, (n,), jnp.nan)
