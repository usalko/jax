...
class PJitErrorTest(jtu.JaxTestCase):
  ...
  def testUndefinedResourcesConstraint(self, mesh, resources):
    ...
    with self.assertRaisesRegex(
        ValueError,
        r"Resource axis: x of.*" + spec_regex(
            pxla.array_mapping_to_axis_resources(pxla._get_array_mapping(
                spec))) + " is undefined"):
  ...
  def testRankTooLowArgs(self):
    ...
    error = re.compile(
        ...
        r".*rank of at least 2, but the aval rank is 1", re.M | re.S)
    with self.assertRaisesRegex(ValueError, error):
  ...
  def testRankTooLowOuts(self):
    ...
    error = re.compile(
        ...
        r".*rank of at least 2, but the aval rank is 0", re.M | re.S)
    with self.assertRaisesRegex(ValueError, error):
