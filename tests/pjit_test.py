...
class ArrayPjitTest(jtu.JaxTestCase):
  ...
  def test_partial_unspecified_axis_resources_error(self):
    with jax._src.config.jax_array(True):
      with self.assertRaisesRegex(
        ValueError, ('`pjit._UNSPECIFIED` exists in out_axis_resources. Make '
                     'sure that every entry in out_axis_resources is '
                     '`pjit._UNSPECIFIED`.')):
        pjit(lambda x, y: (x, y), out_axis_resources=(P('x'), pjit_lib._UNSPECIFIED))

      with self.assertRaisesRegex(
        ValueError, ('`pjit._UNSPECIFIED` exists in in_axis_resources. Make '
                     'sure that every entry in in_axis_resources is '
                     '`pjit._UNSPECIFIED`.')):
        pjit(lambda x, y: (x, y), in_axis_resources=(P('x'), pjit_lib._UNSPECIFIED))
